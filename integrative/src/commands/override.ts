...
export async function overrideCommand(
  ...
  const override = {
    pr: prRef,
    originalScore: parseInt(options.originalScore ?? '0', 10),
    originalConfidence: 0,
    originalRationale: options.originalRationale ?? 'Not provided',
    overrideScore: parseInt(options.score, 10),
    reason: options.reason,
    timestamp: new Date().toISOString(),
  };

  saveOverride(override);
  console.log(chalk.green(`Override saved for ${prRef}: ${override.originalScore} -> ${override.overrideScore}`));
}
