...
export async function gapsCommand(): Promise<void> {
  ...
  if (overrides.length === 0) {
    console.log(chalk.dim('No overrides found. Use `pr-scorer override` to record score disagreements.'));
    return;
  }
  ...
  if (unanalyzed.length > 0) {
    ...
    for (const override of unanalyzed) {
      try {
        ...
        console.log(`  ${chalk.bold(override.pr)}: ${override.originalScore} -> ${override.overrideScore}`);
