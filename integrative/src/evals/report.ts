...
function analyzeGaps(results: FixtureResult[]): GapAnalysisItem[] {
	...
	if (overScored.length >= 3) {
		gaps.push({
			...
			suggestedFix:
				"Review scoring prompt to be more conservative. Model may be overweighting certain signals.",
			priority: "high",
		});
	}
	...
	if (underScored.length >= 3) {
		gaps.push({
			...
			suggestedFix:
				"Review scoring prompt to better recognize impact. Model may be missing architectural significance.",
			priority: "high",
		});
	}
	...
	if (largeDelta.length >= 1) {
		gaps.push({
			...
			suggestedFix:
				"These fixtures may need expected score review, or represent edge cases needing special handling.",
			priority: "high",
		});
	}
